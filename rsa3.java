...
public class rsa3 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("enter key gen choice1.random/n2.user");
        int ch=sc.nextInt();
        int[] user=new int[2];
        switch (ch) {
            case 1 -> user=randomgen(rand);

            case 2 -> user=usergen(sc);
            default -> throw new AssertionError();
        }
       
        // 2. Calculate
        int n = user[0] * user[1];
        int m = (user[0] - 1) * (user[1] - 1);
    ...
    static int[] randomgen(Random rand){
